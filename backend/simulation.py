...
def draw_outcome_indices(cumulative_probs: np.ndarray, shape: tuple[int, ...], rng: np.random.Generator) -> np.ndarray:
    """Draws outcome indices of the given shape from a cumulative probability array.
    ...
    and then picking a uniform card within that tier — because a
    cumulative-probability array is built by laying each tier's cards
    end-to-end with the tier's probability split evenly across them (see
    distribution.py). Landing a uniform draw in a given interval of the
    cumulative array is exactly equivalent to that two-stage draw, just
    flattened into a single array lookup that numpy can vectorize instead of
    looping per pack.

    Takes the cumulative array directly (rather than a SetDistribution)
    specifically so this one sampling primitive can be reused unchanged by
    importance_sampling.py, which needs to draw from a *different*
    (tilted) cumulative array over the same set of outcomes — the sampling
    mechanics are identical either way, only which array is passed in
    changes.
    ...
    return np.searchsorted(cumulative_probs, uniforms, side="right")
...
def simulate_pack_profits(dist: SetDistribution, n_trials: int, rng: np.random.Generator) -> np.ndarray:
    ...
    """
    indices = draw_outcome_indices(dist.cumulative_probs, (n_trials,), rng)
...
def simulate_y_pack_profits(dist: SetDistribution, y: int, n_trials: int, rng: np.random.Generator) -> np.ndarray:
    ...
    """
    indices = draw_outcome_indices(dist.cumulative_probs, (n_trials, y), rng)
